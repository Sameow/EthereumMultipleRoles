...
contract("Successful sale", async accounts => {
    let token;
    let ecosystem;
    let listPrice = 2000;
    ...
    it("Inspector accepts.", async () => {
        await ecosystem.acceptSale(0, {from: inspector});
        let actualAnswer = await token.balanceOf.call(seller);
        assert.equal(actualAnswer.valueOf(), 2000, "Seller should have earned 2000.");
        actualAnswer = await token.balanceOf.call(ecosystem.address);
